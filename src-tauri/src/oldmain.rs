...
fn listar_caixas(database: tauri::State<'_, Mutex<BancoDeDados>>)->String{
    serde_json::to_string(&executor::block_on(database.lock().unwrap().listar_caixas())).unwrap()    
}

#[tauri::command]
fn listar_tipos_pagamento(database: tauri::State<'_, Mutex<BancoDeDados>>)->String{
    serde_json::to_string(&executor::block_on(database.lock().unwrap().listar_tipos_pagamento())).unwrap()
}

#[tauri::command]
fn listar_setores(database: tauri::State<'_, Mutex<BancoDeDados>>)->String{
    serde_json::to_string(&executor::block_on(database.lock().unwrap().listar_setores())).unwrap()
}

#[tauri::command]
fn listar_empresas(database: tauri::State<'_, Mutex<BancoDeDados>>)->String{
    serde_json::to_string(&executor::block_on(database.lock().unwrap().listar_empresas())).unwrap()
}

#[tauri::command]
fn validar_gasto(database: tauri::State<'_, Mutex<BancoDeDados>>,json_data: &str)->String{
    serde_json::to_string(&executor::block_on(database.lock().unwrap().validar_gasto_de_json(json_data))).unwrap()
}

#[tauri::command]
fn registrar_gasto(database: tauri::State<'_, Mutex<BancoDeDados>>,json_data: &str)->String{
    serde_json::to_string(&executor::block_on(database.lock().unwrap().registrar_gasto_de_json(json_data))).unwrap()
}
...
fn main() {
    ...
    let database = Mutex::new( 
        match executor::block_on(storage::BancoDeDados::abrir()) {
            ...
            Err(_)=>{
                ...
                "C:/Users/titog/Documentos/raja.db",
                ))).unwrap();
    ...
    println!("{}",serde_json::to_string(&executor::block_on(database.lock().unwrap().listar_empresas())).unwrap());

    tauri::Builder::default()
        .manage(database)
        .invoke_handler(tauri::generate_handler![listar_caixas,listar_tipos_pagamento,listar_setores,listar_empresas,validar_gasto,registrar_gasto])
